...
export const userList = (params: Type, url: string) => {
    let listEntriesQuery = `SELECT id, name, sender,id_number FROM black_list ${keyWhere(params.key)}${orderBy(params.direction, params.column)} ${params.limitQuery}`;

    return query(listEntriesQuery, url);
}

export const countList = () => {
    return query("SELECT COUNT(*) AS counts FROM black_list")
}

export const listByid = (id: number) => {
    let queryUserbyId = `SELECT * FROM black_list WHERE id = ?`;
...
export const insertList = (name: string, sender: string, idNumber: number) => {
    let queryInsert = `INSERT INTO black_list(name,sender, id_number) VALUES (?,?,?)`;
    return query(queryInsert, [name, sender, idNumber]);
};

export const editList = (name: string, sender: string, idNumber: number, id: number) => {
...
export const updateUserId = (id: number, userId: number) => {
    return query("UPDATE black_list SET id_user = ? WHERE id = ?", [userId, id])
}